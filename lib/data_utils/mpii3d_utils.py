...
def read_test_data(dataset_path,left_tran=False,right_tran=False,Trans_coef=0.9):
    ...
    user_list = [6]
    count_iter=0
    five_bit_list=[0,0,0,0,0]
    for user_i in user_list:
        print('Subject', user_i+1)
        # seq_path = os.path.join(dataset_path,
        ...
        anno_file = osp.join(
            'G:\\document\\Time-space-map-and-point-cloud-map\\3D-action-estimate\\multiview-human-pose-estimation-pytorch-master\\data\\',
            'mpi-inf-3dhp\\annot\\',
            'train_S{}.pkl'.format(str(user_i+1)))
        with open(anno_file, 'rb') as f:
            mat_as_h5 = pickle.load(f)
        for index in range(len(mat_as_h5)):
            print(mat_as_h5[index]['camera_id'])
        ...
        # mat_as_h5_one_hot = [[], [], [], []]
        # for index1 in range(period_num):
        #     #此处循环选择四个视图中最合适的那个
        #     for index2 in range(4):
        #         if(mat_as_h5[index1+(index2*period_num)]['joints_2d'].min()<0 or mat_as_h5[index1+(index2*period_num)]['joints_2d'].min()>2048 or mat_as_h5[index1+(index2*period_num)]['joints_2d'].max()<0 or mat_as_h5[index1+(index2*period_num)]['joints_2d'].max()>2048):
        #             joint_record=[]
        #             count_joint=0
        #             for joint_value in mat_as_h5[index1 + (index2 * period_num)]['joints_2d']:
        #                 if(joint_value[0]>2048 or joint_value[0]<0 or joint_value[1]<0 or joint_value[1]>2048):
        #                     joint_record.append(count_joint)
        #                 count_joint += 1
        #             if(len(joint_record)>=3):
        #                 mat_as_h5_one_hot[index2].append(np.array([0,0,0,0,1]))
        #                 five_bit_list[4]+=1
        #             else:
        #                 if(len(joint_record)==2):
        #                     if(joint_record in connectjoint_just):
        #                         mat_as_h5_one_hot[index2].append(np.array([0,0,0,1,0]))
        #                         five_bit_list[3] += 1
        #                     else:
        #                         mat_as_h5_one_hot[index2].append(np.array([0,0,1,0,0]))
        #                         five_bit_list[2] += 1
        #                 else:
        #                     mat_as_h5_one_hot[index2].append(np.array([0,1,0,0,0]))
        #                     five_bit_list[1] += 1
        #         else:
        #             mat_as_h5_one_hot[index2].append(np.array([1,0,0,0,0]))
        #             five_bit_list[0] += 1
        #             continue
        # mat_as_h5_one_hot=np.array(mat_as_h5_one_hot[0][:]+mat_as_h5_one_hot[1][:]+mat_as_h5_one_hot[2][:]+mat_as_h5_one_hot[3][:])
        # print(five_bit_list)
        # if (count_iter == 0):
        #     mat_as_h5_one_hot_top = np.array(mat_as_h5_one_hot)
        # else:
        #     mat_as_h5_one_hot_top = np.concatenate((mat_as_h5_one_hot_top, np.array(np.array(mat_as_h5_one_hot))), axis=0)

        # mat_as_h5_one_hot_top = np.array(mat_as_h5_one_hot_top)
        # # mat_as_h5_one_hot_sorted = sorted(mat_as_h5_one_hot_top)
        # mat_as_h5_one_hot_sorted = mat_as_h5_one_hot_top.sort()
        # figsize = 11, 9
        # figure, ax = plt.subplots(figsize=figsize)
        # x3 = range(len(mat_as_h5_one_hot_top))
        # plt.ylim(0, 200)  # 限定纵轴的范围
        # plt.xlim(0, len(mat_as_h5_one_hot_top))
        # plate =plt.plot(x3, mat_as_h5_one_hot_top, marker='o', mec='r', mfc='w', label=u'Circumference')
        # font1 = {'family': 'Times New Roman',
        #          'weight': 'normal',
        #          'size': 23,
        #          }
        # plt.legend(handles=plate, prop=font1)
        # # plt.legend()  # 让图例生效
        # plt.tick_params(labelsize=23)
        # labels = ax.get_xticklabels() + ax.get_yticklabels()
        # [label.set_fontname('Times New Roman') for label in labels]
        # plt.xlabel(u"samples", fontsize=15)  # X轴标签
        # plt.ylabel("", fontsize=15)  # Y轴标签plt.ylabel("Micro/Macro-F;samples;weighted;accuracy")
        # plt.title(u"", fontsize=15)  # 标题#plt.title(u"wikipedia(treated)")
        # # plt.savefig('./1.jpg')
        # plt.show()
            # mat_as_h5_con2=[]
        ...
        # Inner_Outer_sum=[]
        # mat_as_IO_one_hot=[]
        # for index1 in range(period_num*4):
        #     #此处循环选择四个视图中最合适的那个
        #     data_convert_joint=[]
        ...
        #         trans = get_affine_transform(center,scale, rotation, np.array([int(256),int(256)]))
        ...
        # plt.ylim(200, 2000)  # 限定纵轴的范围
        # plt.xlim(0, len(Inner_Outer_sum))
        # plate=plt.plot(x3, Inner_Outer_sum, marker='o', mec='r', mfc='w', label=u'Circumference')
        # font1 = {'family': 'Times New Roman',
        #          'weight': 'normal',
        #          'size': 23,
        #          }
        ...
        # [label.set_fontname('Times New Roman') for label in labels]
        # # plt.legend()  # 让图例生效
        # plt.xlabel(u"Samples", fontsize=24)  # X轴标签
        # plt.ylabel("Sum of inner and outer Circumference", fontsize=24)  # Y轴标签plt.ylabel("Micro/Macro-F;samples;weighted;accuracy")
        # # plt.title(u"", fontsize=15)  # 标题#plt.title(u"wikipedia(treated)")
        # # plt.savefig('./1.jpg')
        # plt.show()
        # figsize = 9.5, 7.773
        # figure, ax = plt.subplots(figsize=figsize)
        # x3 = range(len(Inner_Outer_sum_sorted))
        # plt.ylim(200, 2000)  # 限定纵轴的范围
        # plt.xlim(0, len(Inner_Outer_sum_sorted))
        # plt.plot(x3, Inner_Outer_sum_sorted, marker='o', mec='r', mfc='w', label=u'Circumference(sorted)')
        # plt.legend(handles=plate, prop=font1)
        # plt.tick_params(labelsize=23)
        # labels = ax.get_xticklabels() + ax.get_yticklabels()
        # [label.set_fontname('Times New Roman') for label in labels]
        ...
        for num in tqdm(enumerate(valid)):
            ...
            img_i = os.path.join('F:\\MPI-INF-3DHP(beifen)\\MPI-INF-3DHP(beifen)\\',
                                 valid_i+'\\',
                                 frame_i)
            ...
            joints_2d = convert_kps(joints_2d_raw, src="mpii3d_test", dst="spin").reshape((-1, 3))#最后得到了关节顺序与spin一样的关节顺序，但是中间可能有间隔，也不是从第一个开始
            ...
            joints_3d = convert_kps(joints_3d_raw, "mpii3d_test", "spin").reshape((-1, 3))#最后得到了关节顺序与spin一样的关节顺序，但是中间可能有间隔，也不是从第一个开始
        ...
        for i in tqdm(range(0,index_1000,1000)):
            if(index_1000-i<=1000):
                features = extract_features(model, np.array(vid_used_frames)[i:index_1000],
                                            ...
                                            dataset='spin', debug=False,left_tran=left_tran,right_tran=right_tran,Trans_coef=Trans_coef)
    ...
    # save_file1.to_csv('C:\\MPI-INF-3DHP(beifen)\\video_dir\\iris_training1_MPI_S6.csv')
    # save_file2.to_csv('C:\\MPI-INF-3DHP(beifen)\\video_dir\\iris_training2_MPI_S6.csv')
    for k in dataset.keys():
        dataset[k] = np.array(dataset[k])
    dataset['features'] = np.concatenate(dataset['features'])
...
if __name__ == '__main__':
    ...
    right_tran = True
    Trans_coef = 0.98
    dataset = read_test_data(args.dir,left_tran=left_tran,right_tran=right_tran,Trans_coef = Trans_coef)
    if(left_tran):
        joblib.dump(dataset, osp.join(VIBE_DB_DIR, 'mpii3d_val_db_MPI_S7_left_trans_0.9.pt'))
    elif(right_tran):
        joblib.dump(dataset, osp.join(VIBE_DB_DIR, 'mpii3d_val_db_MPI_S7_right_trans_0.9.pt'))
    joblib.dump(dataset, osp.join(VIBE_DB_DIR, 'mpii3d_val_db_MPI_S7.pt'))
