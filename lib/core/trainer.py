...
def get_single_item(db,index,batch_size):
    ...
    for batch_index in range(batch_size):
        ...
        # input_left = torch.from_numpy(db.dataset.db_left['features'][start_index:end_index + 1]).float()
        # input_right = torch.from_numpy(db.dataset.db_right['features'][start_index:end_index + 1]).float()
...
class Trainer():
    def __init__(
        ...
        self.train_2d_iter = self.train_loader

        # if self.train_3d_loader:
            # self.train_3d_iter = iter(self.train_3d_loader)
        self.train_3d_iter = self.train_loader
    ...
    def train(self):
        ...
        for i in range(self.num_iters_per_epoch):
            for i in range(0, len(self.train_3d_iter.dataset.vid_indices),int(self.train_3d_iter.batch_size * 10)):
                target = get_single_item(self.train_3d_iter, i, self.train_3d_iter.batch_size)  # 一次性取128个样本
                target_2d = target['kp_2d']
                target_3d = target['kp_3d']
                real_motion_samples = target['kp_3d']
            ...
            #     real_motion_samples = next(self.disc_motion_iter)
                ...
                print("特征图部分的loss为" + str(total_loss.item()))
