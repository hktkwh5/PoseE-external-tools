...
def batch_compute_similarity_transform_torch(S1, S2):
    ...
    transposed = False
    if S1.shape[0] != 3 and S1.shape[0] != 2:
        S1 = S1.permute(0,2,1)#该方法调换矩阵的维度
        S2 = S2.permute(0,2,1)
        transposed = True
    assert(S2.shape[1] == S1.shape[1])

    # 1. Remove mean.
    mu1 = S1.mean(axis=-1, keepdims=True)#将14个结点的进行了平均。最后的形状是batch*xyz*1，之所以还是三个维度，是因为keepdims=True
    mu2 = S2.mean(axis=-1, keepdims=True)

    X1 = S1 - mu1#这里使用所有关节在xyz轴上的相对于平均偏移量的差值。
    X2 = S2 - mu2

    # 2. Compute variance of X1 used for scale.
    var1 = torch.sum(X1**2, dim=1).sum(dim=1)#先将矩阵中的数值进行平方，然后将xyz维进行求和并且消去该维，然后对14个关节的维进行求和，最后消去该维

    # 3. The outer product of X1 and X2.
    K = X1.bmm(X2.permute(0,2,1))#函数的传入参数很简单，两个三维矩阵而已，只是要注意这两个矩阵的shape有一些要求：

    # 4. Solution that Maximizes trace(R'K) is R=U*V', where U, V are
    # singular vectors of K.
    U, s, V = torch.svd(K)#其中U是一个m*m的矩阵，s是一个m*n的矩阵，除了主对角线上的元素以外全为0，主对角线上的每个元素都称为奇异值，V是一个n*n的矩阵。 U和V都是酉矩阵，即满足

    # Construct Z that fixes the orientation of R to get det(R)=1.
    Z = torch.eye(U.shape[1], device=S1.device).unsqueeze(0)
    Z = Z.repeat(U.shape[0],1,1)
    Z[:,-1, -1] *= torch.sign(torch.det(U.bmm(V.permute(0,2,1))))#torch.sign函数的作用就是输出input通过sign函数后的张量，其中sign函数就是符号函数，

    # Construct R.
    R = V.bmm(Z.bmm(U.permute(0,2,1)))

    # 5. Recover scale.
    scale = torch.cat([torch.trace(x).unsqueeze(0) for x in R.bmm(K)]) / var1#torch.trace对角线元素之和。

    # 6. Recover translation.
    t = mu2 - (scale.unsqueeze(-1).unsqueeze(-1) * (R.bmm(mu1)))#unsqueeze函数是在倒数第一个位置上增加一个维度。

    # 7. Error:
    S1_hat = scale.unsqueeze(-1).unsqueeze(-1) * R.bmm(S1) + t#S1_hat应该是估计值
